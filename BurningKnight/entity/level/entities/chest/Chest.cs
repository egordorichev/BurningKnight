...
namespace BurningKnight.entity.level.entities.chest {
	public class Chest : SaveableEntity {
		...
		public override void Init() {
			...
			Body = World.CreateSimpleBody(this, 4, 8, 8, 1, BodyDef.BodyType.DynamicBody, false);
			Sensor = World.CreateSimpleBody(this, 0, 0, 16, 12, BodyDef.BodyType.DynamicBody, true);
			World.CheckLocked(Body).SetTransform(this.X, this.Y, 0);
			Sensor.SetTransform(this.X, this.Y, 0);
		...
		public static Pool MakePool<Item>(ItemRegistry.Quality Quality, bool Weapon, bool Any) {
			...
			foreach (Map.Entry<string, ItemRegistry.Pair> Both in ItemRegistry.Items.EntrySet()) {
				...
				if (ItemRegistry.Check(Item.Quality, Quality) && (Any || Weapon == WeaponBase.GetType().IsAssignableFrom(Item.Type)) && Item.Unlocked(Both.GetKey()) &&
				    (Player.Instance == null || Player.Instance.GetInventory().FindItem(Item.Type) == null))
		...
		public override void OnCollision(Entity Entity) {
			...
			if (Entity is Player) {
				if (Locked) {
					...
					Open = true;
					PlaySfx("chest");
					Data = this.GetOpenAnim();
					Data.SetListener(new AnimationData.Listener {
