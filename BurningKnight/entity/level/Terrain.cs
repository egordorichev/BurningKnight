...
namespace BurningKnight.entity.level {
	public class Terrain {
		...
		public static TextureRegion[][] Edges = new TextureRegion[16][
		...
		public static TextureRegion[][] Variants = new TextureRegion[16][
		...
		public static Color GetColor(byte T) {
			if (T < 0) return Color.WHITE;

			if (T == WALL || T == CRACK) return Level.Colors[Dungeon.Level.Uid];
			...
			if (T == FLOOR_D) return Floors[Lastt][3];
		...
		public static void LoadTextures(int Set) {
			...
			Decor =  {
				Graphics.GetTexture("props-decor_a"), Graphics.GetTexture("props-decor_b"), Graphics.GetTexture("props-decor_c"), Graphics.GetTexture("props-decor_d"), Graphics.GetTexture("props-decor_e"), Graphics.GetTexture(
					"props-decor_f"), Graphics.GetTexture("props-decor_g")
			}
			;
			...
			for (var I = 0; I < 3; I++) Sides[I] = Graphics.GetTexture(Bm + "-side " + I);

			for (var I = 0; I < 10; I++) Dither[I] = Graphics.GetTexture("fx-dither-idle-" + string.Format("%02d", I));

			for (var I = 0; I < 16; I++) Spread[I] = Graphics.GetTexture("biome-gen-l" + Level.COMPASS[I]);

			for (var I = 0; I < 16; I++) Pooledge[I] = Graphics.GetTexture("biome-gen-pooledge" + Level.COMPASS[I]);

			for (var I = 0; I < 16; I++) Lavaedge[I] = Graphics.GetTexture("biome-gen-lavaedge" + Level.COMPASS[I]);

			for (var I = 0; I < 16; I++) Obedge[I] = Graphics.GetTexture("biome-gen-ob" + Level.COMPASS[I]);

			for (var I = 0; I < 16; I++) Webedge[I] = Graphics.GetTexture("biome-gen-web" + Level.COMPASS[I]);

			for (var I = 0; I < 16; I++) Grassedge[I] = Graphics.GetTexture("biome-gen-grassedge" + Level.COMPASS[I]);

			for (var I = 0; I < 16; I++) Drygrassedge[I] = Graphics.GetTexture("biome-gen-dry" + Level.COMPASS[I]);

			for (var I = 0; I < 16; I++) Dirtedge[I] = Graphics.GetTexture("biome-gen-dirtedge" + Level.COMPASS[I]);

			for (var I = 0; I < 16; I++) TableVariants[I] = Graphics.GetTexture(Bm + "-platform A" + Level.COMPASS[I]);

			for (var I = 0; I < 16; I++) WoodVariants[I] = Graphics.GetTexture(Bm + "-floor B " + string.Format("%02d", I + 1));

			for (var I = 0; I < 16; I++) FloorVariants[I] = Graphics.GetTexture(Bm + "-floor A " + string.Format("%02d", I + 1));

			for (var I = 0; I < 16; I++) GoldVariants[I] = Graphics.GetTexture(Bm + "-floor D " + string.Format("%02d", I + 1));

			for (var I = 0; I < 16; I++) BadVariants[I] = Graphics.GetTexture(Bm + "-floor C " + string.Format("%02d", I + 1));

			for (var I = 0; I < 12; I++) TopVariants[I] = Graphics.GetTexture(Bm + "-wall " + Letters[I / 4] + " " + string.Format("%02d", I % 4 + 1));

			for (var I = 0; I < 4; I++) {
				ChasmSides[I] = new TextureRegion[3];

				for (var J = 0; J < 3; J++) ChasmSides[I][J] = Graphics.GetTexture("biome-gen-edge_" + Coords[I] + "_" + (J + 1));
			}
