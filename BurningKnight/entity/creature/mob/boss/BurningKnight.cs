	...
	public class BurningKnight : Boss {
		...
		public class BKState : State<BurningKnight> {
			public override void Update(float Dt) {
				if (Self.Target != null) Self.LastSeen = new Point(Self.Target.X, Self.Target.Y);
		...
		public class RoamState : BKState {
			...
			public override void Update(float Dt) {
				...
				if (RoomToVisit == null) {
					...
					RoomToVisit = Room.GetCenter();
					RoomToVisit.Mul(16);
					Self.Last = Room;
				}

				if (RoomToVisit != null) {
					if (FlyTo(RoomToVisit, Self.Speed * 5, 32f)) {
						if (Random.Chance(25)) {
							Self.Become("idle");
		...
		public class LChaseState : BKState {
			...
			public override void Update(float Dt) {
				if (FlyTo(Player.Instance, 20, 24f)) {
				}
		...
		public class ChaseState : BKState {
			...
			public override void Update(float Dt) {
				if (FlyTo(Player.Instance, 30, 16f)) {
					Self.Become("explode");
		...
		public class PreattackState : BKState {
			public override void Update(float Dt) {
				...
				if (I == 0) Pattern = Random.NewInt(3);
		...
		public class AttackState : BKState {
			...
			public override void Update(float Dt) {
				...
				if (!Attacked) {
					...
					if (Random.Chance(30)) {
						var A = Random.NewFloat(0, Math.PI * 2);
						float D = 128;
