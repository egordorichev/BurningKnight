...
object={}
 function object:extend(kob)
  -- printh(type(kob))
  if kob and type(kob)=="string" then kob=parse(kob) end
  kob=kob or {}
  kob.extends=self
...
function b(y,xx,yy) 
 for c=0,31 do
  for d=1,31 do 
   ...
   if m~=0 then pset(d+48+xx,c-48+y+yy,m) end
  end 
 end
end 
...
function f(e)
 ...
 for xx=-1,1 do
 for yy=-1,1 do
  if abs(xx)+abs(yy)==1 then b(y,xx,yy) end
 end
 end
 pl(e)
 b(y,0,0)
...
function block_type(blk)
 --if fget(blk,0) then return solid end
 --if fget(blk,1) then return support end
end
 ...
 for x=0,s.x-1 do
  for y=0,s.y-1 do
   ...
   if cl then
    ...
    if not e.norem then mset(xx,yy,e.dtile) end
    e_add(e)
    blk=0
   end
...
function snow:render()
 self.pos.x = self.pos.x + self.sz*0.5
...
function guy:move()
 ...
 self.pos.x = self.pos.x + self.mx*s
 self.pos.y = self.pos.y + self.my*s
end
-->8
-- fx
...
function part:idle()
 self.r = self.r - (self.spd~=nil and self.spd(self.t) or 0.1)
 self.vel = self.vel * (self.mul or 0.9)
 if self.r<0 then
  self.done=true
 end
end
...
["quad_in_out"]=function(t) t=t*2 if t<1 then return 0.5*t*t end
    return -0.5*((t-1)*(t-3)-1) end,
["back_in"]=function(t) local s=back  return t*t*((s+1)*t-s) end,
["back_out"]=function(t) local s=back t = t - 1 return t*t*((s+1)*t+s)+1 end,
["back_in_out"]=function(t) local s=back t = t * 2 if t<1 then s = s * 1.525 return 0.5*(t*t*((s+1)*t-s)) end
 t = t - 2 s = s * 1.525  return 0.5*(t*t*((s+1)*t+s)+2) end
}
