namespace BurningKnight.ui {
	public class UiTextInput : UiButton, InputProcessor {
		...
		public override void Update(float Dt) {
			...
			if (Org.Rexcellentgames.Burningknight.Game.Input.Input.Instance.WasPressed("pause")) OnBackSpace();
		}
		...
		public override bool KeyDown(int Keycode) {
			if (Active) {
				if (Keycode == Input.Keys.BACKSPACE && Input.Length() > 0) {
					...
					var Len = GetMaxLength();

					if (Len != -1 && Input.Length() >= Len) return false;
					...
					if (Len != -1 && Str.Length() + Input.Length() > Len) Str = Str.Substring(0, Len - (Str.Length() + Input.Length()));
