	...
	public class NpcSaveRoom : SpecialRoom {
		...
		public override void Paint(Level level) {
			...
			if (d.X == Left || d.Y == Right) {
				...
				var dr = new VerticalDoor();
				dr.Center = door * 16 + new Vector2(16, 0);
				level.Area.Add(dr);
				
				var v = (d.X == Left ? -1 : 1);
				
				Painter.DrawLine(level, new Vector2(Left + w + v, Top), new Vector2(Left + w + v, Bottom), Tile.WallA);
				...
				Painter.DrawLine(level, new Vector2(Left, Top + h), new Vector2(Right, Top + h), Tiles.Pick(Tile.Chasm, Tile.Lava, Tile.SensingSpikeTmp));
				Painter.Set(level, door, fl);
