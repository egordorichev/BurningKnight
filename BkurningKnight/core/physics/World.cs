...
namespace BurningKnight.core.physics {
	public class World {
		...
		private static Void SetBits(FixtureDef Fixture, Entity Owner) {
			if (!(Owner is SolidProp) && (Fixture.IsSensor || Owner is HeartFx || Owner is Upgrade || Owner is Exit || Owner is PoisonFx || Owner is ItemHolder || Owner is Item || Owner is Projectile || Owner is Shell || Owner is ManaFx || Owner is BurningMan || Owner is Mage || Owner is IceElemental)) {
