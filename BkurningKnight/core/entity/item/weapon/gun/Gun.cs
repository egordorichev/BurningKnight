namespace BurningKnight.core.entity.item.weapon.gun {
	public class Gun : WeaponBase {
		...
		public override Void Render(float X, float Y, float W, float H, bool Flipped, bool Back) {
			...
			this.RenderAt(X + W / 2 + (Flipped ? -7 : 7), Y + H / 4 + this.Owner.Z, Back ? (Flipped ? 45 + 90 : 45) : (A + TextureA), this.Origin.X + this.Mod, this.Origin.Y, false, false, TextureA == 0 ? this.Sx : Flipped ? -this.Sx : this.Sx, TextureA != 0 ? this.Sy : (Flipped) ? -this.Sy : this.Sy);
			X = X + W / 2 + (Flipped ? -7 : 7);
