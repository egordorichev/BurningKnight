...
namespace BurningKnight.core.game.input {
	public class Input : InputProcessor, ControllerListener {
		...
		public Void Update() {
			...
			foreach (Map.Entry<string, State> Pair in this.Keys.EntrySet()) {
		...
		public Void PutState(string Id, State State) {
			foreach (string Name in this.Bindings.Get(Id)) {
		...
		public bool IsDown(string Key) {
			...
			foreach (string Id in this.Bindings.Get(Key)) {
		...
		public string GetMapping(string Key) {
			List<string> Keys = this.Bindings.Get(Key);
		...
		public override Void Disconnected(Controller Controller) {
			if (ActiveController == Controller) {
				...
				if (Dungeon.Game.GetState() is InGameState && !Dungeon.Game.GetState().IsPaused()) {
